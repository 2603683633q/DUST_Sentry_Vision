...
bool rm_decision_interfaces__msg__refree__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("rm_decision_interfaces.msg._refree.Refree", full_classname_dest, 41) == 0);
  }
  rm_decision_interfaces__msg__Refree * ros_message = _ros_message;
  {  // robot_hp
    PyObject * field = PyObject_GetAttrString(_pymsg, "robot_hp");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->robot_hp = (uint16_t)PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }
  {  // base_hp
  ...
  {  // color
    ...
    assert(PyLong_Check(field));
    ros_message->color = (uint16_t)PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }
...
{
  /* NOTE(esteve): Call constructor of Refree */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("rm_decision_interfaces.msg._refree");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "Refree");
  ...
  {  // robot_hp
    PyObject * field = NULL;
    field = PyLong_FromUnsignedLong(ros_message->robot_hp);
    {
      int rc = PyObject_SetAttrString(_pymessage, "robot_hp", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
